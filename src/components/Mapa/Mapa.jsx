...
const Mapa = ({ latitudReporte, 
    ...
    const [lat] = useState( latitudReporte || 19.0409511);
    const [lng] = useState( longitudReporte || -98.221976);
    const [zoom] = useState(initialZoom || 17);
    ...
    let [coordenadasbusqueda, setcoordenadasbusqueda] = useState('')
          ...
          map.current = new mapboxgl.Map({
              ...
              map.current.on('load', function() {
                ...
                buscador.current.on("result", function(result) {                  
                  ...
                  setcoordenadasbusqueda(XYbusqueda)
                ...
                axios.get(url, {
                  ...
                  .then(res =>  {
                    setisCharging(false);
                        ...
                        map.current.addSource('layer_location', {
                            type: 'geojson',
                            data: marcadores || res.data
                        });
          
                        map.current.addLayer({
                            'id': 'xample_points',
                            'type': 'circle',
                            'source': 'layer_location',
                            'paint': {
        ...
        switch (busqueda) {
          ...
          case "coordenadasbusqueda":
            if (coordenadasbusqueda.length > 0) {
              borrarMarkers()
              var coordinatesendpoint = coordenadasbusqueda.split(",")
    ...
    return (           
       <div ref={mapContainerGeneral} className={classNames("map-containerGeneral")}>
        <div style={{display:"flex", width:"100%", position:"absolute"}}>
            ...
            {
                luminarias && 
                <Box sx={{zIndex:1, backgroundColor:"white", width:"20%", height:40, m:1,  borderRadius:2}}>
